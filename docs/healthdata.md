---
layout: page
title: "Life Satisfaction and Relationship Status Data Analysis"
permalink: /projects/healthdata
---

This paper was developed for a course on health data analytics. It was co-created with two other health students (cited in the file), who provided the health literature context and helped develop the research question while I handled all of the coding and data analysis in R. The html file was created using Quarto.

<a href="/assets/healthdata.html" target="_blank">⬇ Open / Download</a>

<iframe
  src="/assets/healthdata.html"
  width="100%"
  height="800px"
  style="border: none;">
</iframe>
